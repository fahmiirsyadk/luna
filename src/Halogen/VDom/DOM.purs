...
hydrateElem = EFn.mkEffectFn8 \currentNode (VDomHydrationSpec { vdomSpec: VDomSpec { document }, hydrateAttributes }) hydrate build ns1 name1 as1 ch1 -> do
  ...
  -- When the VDOM has no children, skip child hydration entirely. This handles
  -- elements that manage their content via DOM properties like innerHTML (e.g.
  -- unsafeRawHtml): the SSG DOM has parsed HTML children but the VDOM expects
  -- none. Attribute hydration (below) will re-apply the innerHTML property.
  children <- case Array.null ch1 of
    true -> pure []
    false -> do
      ...
      traverse onChild $ Array.fromFoldable zippedChildren
...
hydrateKeyed = EFn.mkEffectFn8 \currentNode (VDomHydrationSpec { vdomSpec: VDomSpec { document }, hydrateAttributes }) hydrate build ns1 name1 as1 ch1 -> do
  ...
  children <- case Array.null ch1 of
    ...
    false -> do
      ...
      EFn.runEffectFn3 Util.strMapWithIxE (Array.fromFoldable zippedChildren) extractKey onChild
