  ...
  case v of
    ...
    Property propName propValue → do
      case propName of
        ...
        -- innerHTML is set directly rather than checked. The browser's HTML parser
        -- decodes entities (e.g. &quot; → ") when it parses the pre-rendered HTML, so
        -- element.innerHTML returns decoded text while the VDOM still holds the original
        -- encoded string. Checking would always fail for markup that contains entities.
        -- Re-applying is safe: the value is identical content, just browser-normalized.
        "innerHTML" ->
          EFn.runEffectFn3 setProperty propName propValue el
        _ -> do
